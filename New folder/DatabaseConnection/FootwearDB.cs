...
namespace FootWearAssessment.DatabaseConnection
{
    public class FootwearDB
    {
        ...
        public DataTable SelectCategory(FootWearModel footwearModel)
        {
            ...
            SqlCommand sqlCommandOj = new SqlCommand($"select ProductCode,ProductName,Cost from FootWear_GRK where Category= '{ footwearModel.Category}'", sqlConnectionObj);
        ...
        public StatusProperty Fetch(FootWearModel footwearModel)
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter($"select * from FootWear_GRK where ProductCode = {footwearModel.ProductCode} ", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        public string Payment(StatusProperty status)
        {
            SqlConnection con = new SqlConnection(sqlConnectionstr);
            con.Open();
            SqlCommand cmd = new SqlCommand($"insert into OrderPage_RK values({status.ProductCode},'{status.ProductName}',{status.Cost},{status.Quantity},{status.TotalAmount})", con); ;
            cmd.ExecuteNonQuery();
            con.Close();
        ...
        {
            SqlConnection con = new SqlConnection(sqlConnectionstr);
            SqlDataAdapter da = new SqlDataAdapter("select * from OrderPage_RK", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            return dt;
        }
